...
try {
    ...
    if ($staticWebApps -and $staticWebApps.Count -gt 0) {
        ...
        if ($deploymentToken) {
            ...
            Write-Host "Could not retrieve Static Web App deployment token" -ForegroundColor Yellow
            Write-Host "   You'll need to deploy infrastructure first, then get the token manually" -ForegroundColor Yellow
        }
    } else {
        Write-Host "Static Web App not found in resource group: $resourceGroup" -ForegroundColor Yellow
    ...
    Write-Host "Could not check for Static Web App (resource group may not exist yet)" -ForegroundColor Yellow
}
